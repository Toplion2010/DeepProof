...
import { Upload, FileText, ImageIcon, X, ShieldAlert, CheckCircle2, Loader2 } from "lucide-react"
import { Button } from "@/components/ui/button"
import { setUploadedDocument } from "@/lib/document-store"
...
export function DocumentUploadZone() {
  ...
  const handleFile = useCallback((file: File) => {
    ...
    const typeLabel = file.type.startsWith("application/pdf") ? "PDF" : file.type.split("/")[1].toUpperCase()
    setSelectedFile({
      file,
      name: file.name,
  ...
  const handleAnalyze = () => {
    ...
    setState("uploading")
    setUploadedDocument(selectedFile.file)
    router.push("/documents/results")
  }
...
function FileSelectedView({
  ...
  const Icon = file.type === "PDF" ? FileText : ImageIcon
      ...
      <div className="mb-4 w-full max-w-md rounded-lg border border-amber-500/20 bg-amber-500/5 px-4 py-3">
        <label className="flex items-start gap-3 cursor-pointer">
          ...
          <span className="text-xs text-muted-foreground leading-relaxed">
            I consent to send document content and a page image to Groq AI for analysis
          </span>
        </label>
      </div>
  ...
  return (
    <>
      ...
      <p className="text-sm text-muted-foreground">
        Extracting content and running forensic checks...
      </p>
    </>
  )
}
